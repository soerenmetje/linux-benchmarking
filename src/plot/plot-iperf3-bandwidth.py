...
df['upload'] = df['uploadBitsPerSec'] / 1000000  # to MBit/s
df['download'] = df['downloadBitsPerSec'] / 1000000  # to MBit/s
print(df)
...
# Upload
chart = sns.barplot(ax=axes[0], data=df[["project", "upload"]], hue="project",
                    x="project", y="upload",
                    estimator=np.mean, errorbar='sd', capsize=.1, alpha=0.8)
chart.set(xlabel='Machine', ylabel='Throughput  [MBit/s]\nhigher is better', title="Upload")

for i in chart.containers:
    chart.bar_label(i, label_type="center", fmt="%.0f")

# Download
chart = sns.barplot(ax=axes[1], data=df[["project", "download"]], hue="project",
                    x="project", y="download",
                    estimator=np.mean, errorbar='sd', capsize=.1, alpha=0.8)
chart.set(xlabel='Machine', ylabel='Throughput  [MBit/s]\nhigher is better', title="Download")
